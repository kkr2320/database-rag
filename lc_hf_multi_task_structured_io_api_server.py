...
class vectordb_fetch(BaseRetriever):
   def _get_relevant_documents( 
     ...
     with psycopg.connect(connection_string) as conn: 
       ...
       sql_stmt = "SELECT 'date reported:' || doc_date || ' ' || doc_page_content FROM dfs_financial_documents " 
       if vectordb_args is not None :
          sql_stmt = sql_stmt + " WHERE " + vectordb_args + " "
...
pipe = pipeline(model_task, tokenizer=tokenizer, model=llm_model, max_new_tokens=5000 )
